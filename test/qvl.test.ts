...
function buildCRLWithSerials(serialsUpperHex: string[]): Buffer {
  ...
  const tbs = tlv(0x30, Buffer.concat([version, sigAlg, issuer, thisUpdate, revokedSeq]))
...
function getGcpCertPems(): { leaf: string; intermediate: string; root: string; all: string[] } {
  ...
  return { leaf: leafPem, intermediate: intermediatePem, root: rootPem, all: pems }
}

test.serial("Invalid: incorrect pinned root (missing)", async (t) => {
  const quoteB64 = getGcpQuoteBase64()
  const err = t.throws(() =>
    verifyTdxCertChainBase64(quoteB64, [], BASE_TIME),
  )
  t.truthy(err)
...
test.serial("Invalid: revoked root certificate", async (t) => {
  ...
  const rootSerial = new X509Certificate(root).serialNumber.replace(/[^0-9A-F]/g, "").toUpperCase().replace(/^0+(?=[0-9A-F])/g, "")
...
test.serial("Invalid: revoked intermediate certificate", async (t) => {
  ...
  const serial = new X509Certificate(intermediate).serialNumber.replace(/[^0-9A-F]/g, "").toUpperCase().replace(/^0+(?=[0-9A-F])/g, "")
...
test.serial("Invalid: revoked leaf certificate", async (t) => {
  ...
  const serial = new X509Certificate(leaf).serialNumber.replace(/[^0-9A-F]/g, "").toUpperCase().replace(/^0+(?=[0-9A-F])/g, "")
  const crl = buildCRLWithSerials([serial])
  const err = t.throws(() =>
    verifyTdxCertChainBase64(
      quoteB64,
...
test.serial("Invalid: invalid leaf certificate signature", async (t) => {
  ...
  const err = t.throws(() =>
    verifyTdxCertChain(
      ...
      loadRootCerts("test/certs"),
      BASE_TIME,
      [tamperedLeaf, intermediate, root],
    ),
  )
  t.truthy(err)
  t.regex(err!.message, /invalid cert chain/i)
...
test.serial("Invalid: invalid intermediate certificate signature", async (t) => {
  const quoteB64 = getGcpQuoteBase64()
  const quoteBuf = Buffer.from(quoteB64, "base64")
  const { leaf, intermediate, root } = getGcpCertPems()
  ...
  const err = t.throws(() =>
    verifyTdxCertChain(
      ...
      loadRootCerts("test/certs"),
      BASE_TIME,
      [leaf, tamperedIntermediate, root],
    ),
  )
  t.truthy(err)
  t.regex(err!.message, /invalid cert chain/i)
...
test.serial("Invalid: invalid root self-signature", async (t) => {
  ...
  const err = t.throws(() =>
    verifyTdxCertChain(
      ...
      loadRootCerts("test/certs"),
      BASE_TIME,
      [leaf, intermediate, tamperedRoot],
    ),
  )
  t.truthy(err)
  t.regex(err!.message, /invalid cert chain/i)
...
test.serial("Invalid: incorrect quoting enclave signature", async (t) => {
  ...
  const mutated = Buffer.concat([
    ...
    Buffer.from(new Uint8Array([sigData.length & 0xff, (sigData.length >> 8) & 0xff, (sigData.length >> 16) & 0xff, (sigData.length >> 24) & 0xff])),
    sigData,
  ])
  const err = t.throws(() =>
    verifyTdxCertChain(
...
test.serial("Invalid: incorrect public key used to sign the measurement", async (t) => {
  const quoteB64 = getGcpQuoteBase64()
  const original = Buffer.from(quoteB64, "base64")
  const signedLen = getTdxV4SignedRegion(original).length
  const sigLen = original.readUInt32LE(signedLen)
