...
test.serial("Parse a V5 TDX 1.0 attestation", async (t) => {
  const data = JSON.parse(
    fs.readFileSync("test/sample/tdx-v5-tdx1.0-sample.json", "utf-8"),
  )
  const quote: string = data.tdx.quote
  const { header, body } = parseTdxQuoteBase64(quote)
  ...
  t.is(hex(body.mr_td), expectedMRTD)
  t.is(hex(body.report_data), expectedReportData)
  t.false("tee_tcb_svn_2" in body)
})

test.serial("Parse a V5 TDX 1.5 attestation", async (t) => {
  const data = JSON.parse(
    fs.readFileSync("test/sample/tdx-v5-tdx1.5-sample.json", "utf-8"),
  )
  const quote: string = data.tdx.quote
  const { header, body } = parseTdxQuoteBase64(quote)
  ...
  // @ts-ignore extended fields only in 1.5
  t.is(body.tee_tcb_svn_2.length, 16)
  // @ts-ignore extended fields only in 1.5
  t.is(body.mrservictd.length, 48)
})
