...
import { findFreePort, waitForPortOpen, waitForPortClosed } from "../server/utils.js"
...
async function startLauncher(manifestPath: string, port: number): Promise<LauncherProcess> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      if (!resolved) {
        proc.kill()
        reject(new Error(`Launcher failed to start within timeout. stdout: ${stdout}, stderr: ${stderr}`))
      }
    ...
    proc.stdout.on("data", (data) => {
      ...
      if (str.includes("Worker running at") || str.includes("Server listening on")) {
        if (!resolved) {
          resolved = true
    ...
    proc.on("exit", (code) => {
      if (!resolved && code !== 0) {
        clearTimeout(timeout)
        reject(new Error(`Launcher exited with code ${code}. stdout: ${stdout}, stderr: ${stderr}`))
      }
    })
  })
}
