...
test.serial("replicate data written to primary, persist between runs", async (t) => {
  ...
  const replicaValue1 = result1.rows[0].value ?? Object.values(result1.rows[0])[0]
  ...
  t.truthy(result2.rows.length > 0, "replica should still contain data after restart")
  const replicaValue2 = result2.rows[0].value ?? Object.values(result2.rows[0])[0]
  t.is(replicaValue2, "bar", "replica should contain the persisted value after restart")
})
