...
test.before(async () => {
  shared = await startKettleWithTunnel("sgx")
})
...
test.serial("SGX tunnel: GET /uptime", async (t) => {
  if (!shared) t.fail("SGX tunnel not initialized")
...
test.serial("SGX tunnel: POST /increment", async (t) => {
  if (!shared) t.fail("SGX tunnel not initialized")
  const { tunnelClient } = shared!
...
test.serial("SGX tunnel: WebSocket echo", async (t) => {
  if (!shared) t.fail("SGX tunnel not initialized")
...
test.serial("SGX tunnel: WebSocket chat messages", async (t) => {
  if (!shared) t.fail("SGX tunnel not initialized")
  const { tunnelClient, origin } = shared!
