...
export type TunnelClientConfig = {
  // Either measurements or customVerifyQuote is required
  measurements?: MeasurementConfig
  customVerifyQuote?: (quote: TdxQuote | SgxQuote) => Awaitable<boolean>

  // Override default TDX binding: quote.body.report_data == sha512(nonce || iat || userdata)
  x25519Binding?: (client: TunnelClient) => Awaitable<boolean>

  // SGX binding (channels using SGX must implement customVerifyQuote)
  sgx?: boolean

  /**
   * Azure TDX vTPM binding mode.
   *
   * When enabled, verifies the following chain of trust:
   *
   * 1. Intel Root CA → PCK Cert → QE → TDX Quote (via verifyTdx)
   * 2. report_data[0:32] = SHA256(runtime_data JSON)
   * 3. report_data[32:64] = zeros (Azure convention)
   * 4. runtime_data.keys contains HCLAkPub RSA key (presence check)
   * 5. runtime_data["user-data"] = SHA512(nonce || x25519key)
   *
   * ## Why This Is Secure
   *
   * The TDX quote signature covers BOTH the measurements (mr_td, rtmr0-3) AND the
   * report_data in a single signed message. This cryptographically binds:
   * - The code identity (measurements) to the runtime_data hash
   * - The runtime_data (containing HCLAkPub and user-data) to the X25519 key
   *
   * A malicious party cannot mix-and-match: they cannot take a valid quote with
   * measurements X and attach runtime_data from a different machine, because
   * changing report_data would invalidate the quote signature.
   *
   * ## What HCLAkPub Represents
   *
   * HCLAkPub is the vTPM Attestation Key, generated by Microsoft's Paravisor (HCL)
   * at boot. It's a raw JWK embedded in runtime_data. Trust is established through
   * hash binding to the TDX quote, not through vTPM quote verification or a separate
   * cert chain.
   *
   * ## Recommended Enhancements
   *
   * - TCB verification: configurable via verifyTdx options
   * - CRL verification: configurable via verifyTdx options
   * - vTPM quote verification: not implemented
   */
  aztdx?: boolean
}
      ...
      this.ws.onmessage = async (event) => {
        ...
        if (isControlChannelKXAnnounce(message)) {
          try {
            ...
            // Must have at least measurements or customVerifyQuote
